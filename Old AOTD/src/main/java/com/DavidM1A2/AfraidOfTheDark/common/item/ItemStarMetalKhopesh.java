...
public class ItemStarMetalKhopesh extends AOTDChargableSword
{
	...
	{
		if (Minecraft.getMinecraft().player.getCapability(ModCapabilities.PLAYER_DATA, null).isResearched(ResearchTypes.StarMetal))
		{
	...
	public void performChargeAttack(ItemStack itemStack, World world, final EntityPlayer entityPlayer)
	{
		...
		for (Object entityObject : entityList)
		{
			if (entityObject instanceof EntityPlayer || entityObject instanceof EntityLiving)
			{
				...
				entity.attackEntityFrom(DamageSource.causePlayerDamage(entityPlayer), AOTDToolMaterials.StarMetal.getToolMaterial().getAttackDamage() + 4.0F + sharpnessDamage * 1.5F);
		...
		{
			AfraidOfTheDark.INSTANCE.getAOTDExecutor().schedule(new Runnable()
			{
				@Override
				public void run()
				{
