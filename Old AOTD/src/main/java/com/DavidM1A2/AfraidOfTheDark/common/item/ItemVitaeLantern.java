...
public class ItemVitaeLantern extends AOTDItem
{
	...
	{
		if (Minecraft.getMinecraft().player.getCapability(ModCapabilities.PLAYER_DATA, null).isResearched(ResearchTypes.VitaeLanternI))
		{
			tooltip.add("Lantern is " + Math.round(100.0 * NBTHelper.getInt(stack, STORED_VITAE) / ItemVitaeLantern.VITAE_CAPACITY) + "% full.");
			if (Reference.isDebug)
			{
				tooltip.add("Actual vitae = " + this.getStoredVitae(stack) + ", max = " + VITAE_CAPACITY);
			}
		}
		else
		{
