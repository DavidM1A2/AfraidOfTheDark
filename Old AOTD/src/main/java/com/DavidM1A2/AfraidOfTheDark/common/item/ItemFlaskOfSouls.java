...
public class ItemFlaskOfSouls extends AOTDItemWithCooldownPerItem
{
	...
	{
		if (Minecraft.getMinecraft().player.getCapability(ModCapabilities.PLAYER_DATA, null).isResearched(ResearchTypes.PhylacteryOfSouls))
		{
			if (NBTHelper.getString(stack, FLASK_TYPE).equals(""))
			{
			...
			{
				tooltip.add("Entity bound to: " + NBTHelper.getString(stack, FLASK_TYPE).substring(6));
				tooltip.add("Entity kills: " + NBTHelper.getInt(stack, KILLS) + "/" + ItemFlaskOfSouls.flaskKillRequirements.get(NBTHelper.getString(stack, FLASK_TYPE)));
			}
		}
		else
		{
