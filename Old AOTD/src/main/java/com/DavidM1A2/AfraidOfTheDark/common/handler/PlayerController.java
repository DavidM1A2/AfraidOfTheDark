...
public class PlayerController
{
	...
	public void onAttachCapabilitiesEntity(AttachCapabilitiesEvent<Entity> event)
	{
		if (event.getObject() instanceof EntityLivingBase && !(event.getObject() instanceof EntityArmorStand))
			event.addCapability(new ResourceLocation(Reference.MOD_ID + ":entityData"), new AOTDEntityData(event.getObject()));

		if (event.getObject() instanceof EntityPlayer)
			event.addCapability(new ResourceLocation(Reference.MOD_ID + ":playerData"), new AOTDPlayerData((EntityPlayer) event.getObject()));
	}
