...
class PlayerSpellLunarDataCapabilitySerializer : INullableCapabilitySerializable<DoubleTag> {
    private val instance: IPlayerSpellLunarData = PlayerSpellLunarData()

    override fun <V> getCapability(capability: Capability<V>?, side: Direction?): LazyOptional<V> {
        return if (capability == ModCapabilities.PLAYER_SPELL_LUNAR_DATA) LazyOptional.of { instance }.cast() else LazyOptional.empty()
    }
