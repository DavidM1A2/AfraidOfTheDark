...
class WardedBlockMapStorage : Capability.IStorage<IWardedBlockMap> {
    override fun writeNBT(
        ...
        val strengths = ByteArrayNBT(wardedBlocks.values.map { it.ordinal.toByte() }.toByteArray())
        ...
        if (nbt is CompoundNBT) {
            ...
            if (positions.size == strengths.size) {
                for (i in positions.indices) {
                    instance.wardBlock(BlockPos.of(positions[i]), WardStrength.from(strengths[i].toInt()))
                }
            } else {
