...
class ItemCrossbow : AOTDItem("crossbow")
{
        ...
        if (!world.isRemote)
        {
            ...
            else
            {
                ...
                if (!isLoaded(itemStack))
                {
                    ...
                    if (player.capabilities.isCreativeMode || player.inventory.hasItemStack(ItemStack(getCurrentBoltType(itemStack).boltItem)))
                    {
                        // Play the load sound
                        world.playSound(null, player.position, ModSounds.CROSSBOW_LOAD, SoundCategory.PLAYERS, 0.9f, world.rand.nextFloat() * 0.8f + 1.2f)
    ...
    override fun onUsingTick(stack: ItemStack, entity: EntityLivingBase, count: Int)
    {
        ...
        if (entity is EntityPlayer)
        {
            ...
            if (count == 2)
            {
                if (entity.capabilities.isCreativeMode || entity.inventory.clearMatchingItems(getCurrentBoltType(stack).boltItem, -1, 1, null) == 1)
                {
