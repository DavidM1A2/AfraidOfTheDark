 ...
 */
abstract class ResearchRequiredRecipeBase<T : IRecipe>(val baseRecipe: T, private val preRequisite: Research) :
    IForgeRegistryEntry.Impl<IRecipe>(), IRecipe {
    /**
        ...
        private fun findPlayer(inventoryCrafting: InventoryCrafting): EntityPlayer? {
            ...
            // If something goes wrong catch the exception and log it, then return null
                ...
                AfraidOfTheDark.INSTANCE.logger.error(
                    "Could not find the player crafting the recipe, error was: ${ExceptionUtils.getStackTrace(
                        e
                    )}"
                )
            }
