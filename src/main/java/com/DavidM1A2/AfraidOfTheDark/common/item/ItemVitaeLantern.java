...
public class ItemVitaeLantern extends AOTDItem {
	 ...
	 */
	@Override
	public boolean itemInteractionForEntity(ItemStack itemStack, EntityPlayer entityPlayer,
			EntityLivingBase entityLivingBase) {
		if (AOTDPlayerData.get(entityPlayer).isResearched(ResearchTypes.VitaeLanternI)) {
			if (AOTDEntityData.get(entityLivingBase).getVitaeLevel() > 5
				...
				if (AOTDEntityData.get(entityLivingBase).setVitaeLevel(newVitae)) {
					entityLivingBase.worldObj.createExplosion(entityLivingBase, entityLivingBase.getPosition().getX(),
							entityLivingBase.getPosition().getY(), entityLivingBase.getPosition().getZ(), 2, true)
							.doExplosionB(true);
		...
		if (ticksUpdated % UPDATE_FREQUENCY_IN_TICKS == 0) {
			if (NBTHelper.getBoolean(itemStack, "isActive")) {
				if (entity instanceof EntityPlayer) {
					...
					approachEqualibrium(itemStack, entityPlayer,
							MathHelper.floor_double(Constants.entityVitaeResistance.get(EntityPlayer.class)
									* NBTHelper.getDouble(itemStack, "equalibriumPercentage")));
				}
			}
			ticksUpdated = 1;
		} else {
				...
				if (addVitae(itemStack, currentVitae - equalibrium)) {
					if (!entityPlayer.worldObj.isRemote) {
						...
						if (AOTDEntityData.get(entityPlayer).setVitaeLevel(newVitae)) {
							if (!entityPlayer.capabilities.isCreativeMode) {
								entityPlayer.worldObj.createExplosion(entityPlayer, entityPlayer.getPosition().getX(),
										entityPlayer.getPosition().getY(), entityPlayer.getPosition().getZ(), 2,
										entityPlayer.worldObj.getGameRules().getBoolean("mobGriefing"));
								entityPlayer.onKillCommand();
							}
						}
						AOTDEntityData.get(entityPlayer).syncVitaeLevel();
	...
	public void addInformation(final ItemStack itemStack, final EntityPlayer entityPlayer, final List list,
		...
		if (AOTDPlayerData.get(entityPlayer).isResearched(ResearchTypes.VitaeLanternI)) {
			...
			list.add("Press and hold " + Keyboard.getKeyName(Keybindings.changeLanternMode.getKeyCode())
					+ " to set the vitae percentage");
			...
			list.add("Lantern will attempt to keep you at "
					+ Math.round(NBTHelper.getDouble(itemStack, "equalibriumPercentage") * 100)
					+ "% of your maximum vitae.");
			list.add("Lantern is "
					+ Math.round(100.0 * NBTHelper.getInt(itemStack, STORED_VITAE) / ItemVitaeLantern.VITAE_CAPACITY)
					+ "% full.");
		} else {
