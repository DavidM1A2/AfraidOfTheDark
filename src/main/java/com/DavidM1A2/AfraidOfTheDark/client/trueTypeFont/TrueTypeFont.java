...
/**
 * TrueTyper: Open Source TTF implementation for Minecraft. Modified from
 * Slick2D - under BSD Licensing - http://slick.ninjacave.com/license/
 *
...
public class TrueTypeFont {
	...
	private void drawQuad(float drawX, float drawY, float drawX2, float drawY2, float srcX, float srcY, float srcX2,
		...
		worldRenderer.pos(drawX, drawY + DrawHeight, 0.0D).tex((srcX + SrcWidth) / textureWidth, srcY / textureHeight)
				.endVertex();
		// GL11.glTexCoord2f(TextureSrcX, TextureSrcY + RenderHeight);
		// GL11.glVertex2f(drawX, drawY + DrawHeight);

		worldRenderer.pos(drawX + DrawWidth, drawY + DrawHeight, 0.0D).tex(srcX / textureWidth, srcY / textureHeight)
				.endVertex();
		// GL11.glTexCoord2f(TextureSrcX + RenderWidth, TextureSrcY +
		// RenderHeight);
		// GL11.glVertex2f(drawX + DrawWidth, drawY + DrawHeight);

		worldRenderer.pos(drawX + DrawWidth, drawY, 0.0D).tex(srcX / textureWidth, (srcY + SrcHeight) / textureHeight)
				.endVertex();
		// GL11.glTexCoord2f(TextureSrcX + RenderWidth, TextureSrcY);
		// GL11.glVertex2f(drawX + DrawWidth, drawY);

		worldRenderer.pos(drawX, drawY, 0.0D).tex((srcX + SrcWidth) / textureWidth, (srcY + SrcHeight) / textureHeight)
				.endVertex();
	...
	public void drawString(float x, float y, String whatchars, float scaleX, float scaleY, TextAlignment format,
			float... rgba) {
		if (rgba.length == 0)
			rgba = new float[] { 1f, 1f, 1f, 1f };
	...
	public void drawString(float x, float y, String whatchars, int startIndex, int endIndex, float scaleX, float scaleY,
			TextAlignment format, float... rgba) {
		if (rgba.length == 0)
			rgba = new float[] { 1f, 1f, 1f, 1f };
		...
		while (i >= startIndex && i <= endIndex) {
			...
			if (floatObject != null) {
				...
				if (charCurrent == '\n') {
					...
					drawQuad((totalwidth + floatObject.width) * scaleX + x, startY * scaleY + y,
							totalwidth * scaleX + x, (startY + floatObject.height) * scaleY + y,
